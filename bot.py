...
# 1) Dummy Server for Render Port Binding
def run_dummy_server():
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self):
            self.send_response(200)
            self.end_headers()
            self.wfile.write(b"Marjan is Alive and Running")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("السلام عليكم يا ملكي، أنا مرجان.. كيف فيني أخدمك اليوم؟ 💐")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("حصل خطأ بسيط، حاول مرة تانية يا ملكي 💐")
...
def main():
    ...
    logger.info("🚀 مرجان الشامية بدأت العمل بنجاح")
...
# 3) الذاكرة لكل مستخدم
user_chats = {}

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    user_chats[user_id] = model.start_chat(history=[])
    await update.message.reply_text("السلام عليكم يا ملكي، أنا مرجان.. جاهزة لخدمتك بكل دهاء 💐")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"Error: {e}")
        await update.message.reply_text("حصل خطأ صغير، بس مرجان عم تحاول تحله فوراً 💐")
